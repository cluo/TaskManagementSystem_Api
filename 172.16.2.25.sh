...
docker kill redis task-management-mongo; \
    docker rm redis task-management-mongo;
...
docker run -d --link task-management-mongo:mongo --link redis:redis --name=api2 211.157.146.6:5000/task-management-api
docker run -d --link task-management-mongo:mongo --link redis:redis  --link api2:api2 \
